...
export function KundaliChart({ housesArray, planetsInHouses, name = "Customer" }: KundaliChartProps) {
  const chartRef = useRef<HTMLDivElement>(null);
  const [imageUrl, setImageUrl] = useState<string | null>(null);
  const [isGenerating, setIsGenerating] = useState(true);

  // House positions in the chart (0-indexed, matching the array structure)
  // The chart layout: houses are arranged in a specific order visually
  // House 1 (index 0) is at top center, then clockwise
            ...
            <div className="texts" id="texts">
              ...
              <div className="numbers">
                {housePositions.map(({ houseIndex, className }) => {
                  const house = housesArray[houseIndex];
                  const planets = planetsInHouses[houseIndex] || [];
                      ...
                      <span className="single-line">
                        {planets.map((planet, idx) => (
                          <React.Fragment key={idx}>
                            <sup>{planet.sidereal_position.toFixed(2)}</sup> {planet.abbreviation}
                            <br />
                          </React.Fragment>
                        ))}
